...
- 四角共用同一个朝向（图标只表达"这是四分之一"，具体哪一角靠名字）——
  要四角各朝各的就得把模型乘 4，暂时不值

## 贴图是怎么画的

`tools/generate_textures.py` 程序化生成（16×16，和原版同规格），
但**是"作者化像素画"，不是算法噪声**：

- **渐变用 4×4 Bayer 有序抖动**挑色，不用随机噪声 ——
  随机噪声在 16×16 上就是"脏"，有序抖动才像手画的
- **圆用中点画圆（Bresenham）**，1 像素宽。
  "距离落在半径区间里就涂"那种画法在 16×16 上只会得到一圈断续的糊点
- **纹样是手写的像素图**（`MAP_ROUND`，12×12 的字符画）。
  几何算法画装饰纹样必然走样，显式像素图才能控制
- **浮雕感靠"外暗内亮"一对线直接画出来**，
  而不是事后给暗线加内侧高光 —— 那在 2×2 的小花瓣上是破坏性的
- 顶面分三层：外沿压深 → 2 像素**褶边**（每 2 像素一瓣）→ 饼面渐变 + 压纹
- 侧面是**竖褶**，不是几道横杠
- **切面用馅贴图**（`mooncake_filling`）：四分之一块内侧那两个面是豆沙，
  一眼看出是切开的

### 物品图标是平铺的 2D 贴图，不是立体模型

原版的食物（饼干、南瓜派、西瓜片）都是 `item/generated` + 一张 16×16 平铺贴图。
立体模型在 16×16 的格子里只占中间一小块，纹样根本看不清（踩过这个坑：
月饼做成 7×7 的立体块，物品栏里就是个看不清纹路的小方块）。

- 整块：正俯视的圆盘 + 裙边 + 压纹，铺满格子
- 四分之一块：**一个扇形**（圆心在右下），两条直边画成**馅**的颜色 ——
  一眼就知道是从一整块上切下来的一角

预览：`docs/textures_preview.png`、`docs/quarter_preview.png`
